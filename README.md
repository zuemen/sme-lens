...
| **貸後** 早期預警 | 貸後管理倚賴季報，上下游出事才知道 | 以既有逾期戶為種子，在關係圖上做**標籤擴散**（半監督式圖學習），產出帶證據路徑的關注名單 |
...
### `POST /earlywarn` — 貸後早期預警

指定已出事的授信戶（銀行手上本來就有的逾期戶名單），沿關係圖做帶夾制的標籤擴散，
回傳關注名單。

```bash
curl -X POST http://localhost:8000/earlywarn -H 'content-type: application/json' \
  -d '{"seeds": ["宏益企業"], "exposures": {"泰昇精密": 30000000}}'
```

每一筆都帶 `path`（從出事戶到本公司的最短路徑，即證據）、`hops`、`action_zh`
（分級處置建議），並回傳 `exposure_at_risk_twd`——受影響授信餘額合計，刻意排除
種子本身（其餘額已進催收程序，算進來會虛增名單價值）。

納入名單要同時過兩道閘：分數 ≥ 0.20 且跳數 ≤ 2。一份把全圖都列進來的名單等於
沒有名單，而那正是本系統對其他風控工具的批評。
