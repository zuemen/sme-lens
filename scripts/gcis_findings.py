...
def _render_report(
    ...
    add("")
    add(
        "**老實話**：4 家核心子公司（世銓科技、安芯精工、惠智先進、"
        f"{WALKTHROUGH_SUBSIDIARY_NAME}）之間全部是 tier=A、無姓名歧義的"
        "高信心證據，這部分有說服力。但群組其餘成員多來自單一自然人姓名"
        "（tier=B，weight=1，如「李家緯」同時掛名近 10 家顧問／商務公司）——"
        "這類證據沒有身分證字號佐證，可能是同一位專業掛名董事，也可能是"
        "撞名，不能與 A 層等量齊觀。且觀察發現：一詮精密工業本身（母公司）"
        "作為公司節點，並未因為「派員代表自己出任子公司董事」這個事實而"
        "被拉進同一個歸戶群組——build_company_graph 目前的建邊方式只讓"
        "被代表的『子公司之間』互相連結，母公司節點要進同一群組，仍須"
        "額外靠自然人姓名重疊（如本例的周萬順、周孟賢）；這是現有歸戶模型"
        "的已知限制，不是本次修復的範圍，寫出來供下一階段參考。"
    )
    add("")
